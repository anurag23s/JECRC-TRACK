import{ FlatList,StyleSheet, Image, Text, TouchableOpacity, View, SafeAreaView,Button,Dimensions } from 'react-native';
  ...
  return (
    ...
    <LottieView source={require('../../assets/animation_llbv6dqe.json')} autoPlay loop />
     ...
     renderItem={({item })  => (
        ...
        style={tw`p-3 pl-7 pb-8 pt-5 bg-white m-4 w-33 rounded-80 my-20`}
        >
            <View>
            <Image 
                ...
                source={item.image}
            />
            <Text style={tw`mt--1.2 text-lg font-bold`}>{item.title}</Text>
            <Icon  
